---
name: taste-unified
description: "Unified anti-slop design skill. Consolidates all 13 Taste Skill modules into one file. Routes by deliverable (code, images, design spec, audit) and aesthetic profile, enforces the invariants every module agrees on, and scopes the rules the modules disagree on. Covers landing pages, portfolios, redesigns, dashboards, image reference boards, mobile app comps, and brand kits."
---

SIngle-file consolidation of the Taste Skill corpus: 13 skills, 6,670 source lines, merged without deletion.

## GATE 0: MAP, ROUTE, AND HEALTH CHECK (read this first, do not skip)

This file is long and will truncate if you read it top to bottom. Do not do that. Use this map to jump to the exact lines you need, read those in full, then come back here and run the health check before you build. Reading by coordinates is how you avoid truncation. Skipping this map is proof you did not read the skill.

### THE MAP (real line numbers)

- Part 1, router (pick your mode): line 85
- Part 2, universal invariants (always apply): line 136
- Part 3, the dials: line 249
- Part 4, conflict register (read only rows touching your mode): line 269
- Part 9, UNIFIED PRE-FLIGHT / HEALTH CHECK: line 7251

CODE mode profiles:
- A1 design-taste-frontend, THE DEFAULT: line 382
- A2 v1: line 1592
- A3 gpt-taste: line 1822
- A4 redesign-existing (audit phase): line 1900
- A5 high-end-visual-design (soft, premium): line 2082
- A6 minimalist-ui: line 2184
- A7 industrial-brutalist-ui: line 2273
- A8 full-output-enforcement (cross-cutting): line 2369

IMAGE mode profiles:
- B1 imagegen-frontend-web: line 2428
- B2 imagegen-frontend-mobile: line 3419
- B3 brandkit: line 4888

PIPELINE profile: line 5690. SPEC profile: line 6928.

### ROUTE, THEN STOP AND DECLARE

Read Part 1 to pick your mode. Read Part 2. Read the profile the router sends you to, at its line above, IN FULL. Then output this block and STOP, wait for the user before building:

1. DELIVERABLE: [website / one-pager / poster / deck slide / print PDF / dashboard / image board / spec]

2. CANVAS TYPE: [SCROLLING or FIXED]
   FIXED = a bounded page: one-pager, poster, deck slide, print PDF. Page count and page size are a hard boundary.
   If FIXED: fit content by tightening spacing and using full width. Do NOT double padding, do NOT add a hero to fill space, do NOT overflow to extra pages. The profile spacing-inflation rules (double the padding, py-32, py-48, add background imagery) are for SCROLLING pages and are OVERRIDDEN here. On a fixed canvas, restraint means density, not empty space. An empty hero panel or a page padded out to fill a sheet is a failure.
   If SCROLLING: profile spacing rules apply as written.

3. MODE and PROFILE: [mode from Part 1, profile name, and the exact line range you read it from, per the map]

4. BRAND: [the exact colours, fonts, and assets the user gave you. Use these and only these. Never substitute another project's palette. If none were given and the choice is not obvious, ask, do not guess.]

5. PROOF OF READ: [quote one rule, verbatim, from the profile at its line range above. Not from Part 1 or Part 2. From the profile itself. If you cannot quote it, you did not reach it, say so.]

### HEALTH CHECK (run before shipping, not in your head, in the output)

Before you present anything, go to Part 9, read the pre-flight in full, and run it item by item, visibly. Then confirm: canvas type honoured (no inflation on a fixed canvas), brand locked (no foreign palette), profile actually read (line 5 quote is real). Shipping without this shown is not finished.

---

## HOW THIS FILE WORKS (read this first, it is 60 seconds)

The source skills were written to be installed **one at a time**. They never met each other, so they never had to agree. Merged into one file they meet on every line, and they contradict each other in 14 documented places. A flat concatenation would hand you a file that bans Inter on line 400 and recommends it on line 2,100.

This file resolves that with three layers:

1. **PART 2 (Invariants)** holds the rules every module agrees on. These fire in every mode, always, with no override except where an override is explicitly printed.
2. **PART 4 (Conflict Register)** holds every rule the modules disagree on. Each conflict names the winner, the loser, and the condition under which the loser wins instead. Nothing is deleted. The disagreement is made visible and adjudicated rather than silently resolved.
3. **PART 5 onward (Mode Profiles)** holds all 13 source skills verbatim, byte-for-byte, each under its own mode header. When a mode is active, its rules govern anything the Conflict Register hands it.

**Reading order for an agent:** Part 1 (pick mode) → Part 2 (invariants) → Part 3 (dials) → Part 4 (conflicts, only the rows touching your mode) → your mode profile in Part 5+ → Part 6 (pre-flight).

**Precedence, highest to lowest:**

1. Explicit user instruction in the conversation
2. Legal, accessibility, and safety constraints (WCAG AA, regulated-industry requirements)
3. PART 2 Invariants
4. PART 4 Conflict Register rulings
5. The active mode profile
6. The DEFAULT mode profile (`design-taste-frontend` v2)

---

## PART 1: MODE SELECTION (router, run this before anything else)

### 1.1 First question: what is the deliverable?

| The user wants | Mode | Profile section |
|---|---|---|
| Working frontend code | **CODE** | Part 5 |
| Design reference images, no code | **IMAGE** | Part 6 |
| Images first, then code built to match them | **PIPELINE** | Part 7 |
| A `DESIGN.md` spec for Google Stitch or another agent | **SPEC** | Part 8 |
| An existing site made better | **AUDIT** | Part 5, profile A4 |

Do not guess. If the request is "make me a landing page", the deliverable is CODE. If it is "show me what it could look like", the deliverable is IMAGE. If it is "design and build it", the deliverable is PIPELINE.

### 1.2 CODE mode: pick the aesthetic profile

Run the Design Read (Profile A1, Section 0) before picking. The brief picks the profile. Your taste does not.

| The brief reads as | Profile | Install name |
|---|---|---|
| Anything, no strong aesthetic signal. **This is the default.** | **A1** | `design-taste-frontend` (v2) |
| A project pinned to the original v1 behaviour | **A2** | `design-taste-frontend-v1` |
| GPT or Codex is the executing agent, or the user asked for Awwwards-grade GSAP maximalism | **A3** | `gpt-taste` |
| An existing site to audit and upgrade | **A4** | `redesign-existing-projects` |
| Calm, expensive, soft, Apple-adjacent, agency-tier polish | **A5** | `high-end-visual-design` |
| Editorial product UI, Notion or Linear register, restrained | **A6** | `minimalist-ui` |
| Swiss print, terminal, blueprint, declassified, mechanical | **A7** | `industrial-brutalist-ui` |
| The agent keeps truncating and shipping placeholders | **A8** | `full-output-enforcement` |

**One aesthetic profile per project.** A1 through A7 are mutually exclusive. Do not blend A5 (soft, pill buttons, rounded-2rem) with A7 (brutalist, zero radius, 90 degree corners). They are opposite design languages and blending them produces incoherence, not range.

**A8 is the exception.** `full-output-enforcement` is cross-cutting. It stacks on top of any other profile at any time. Apply it whenever output completeness is at risk.

**A4 is a phase, not a rival.** `redesign-existing-projects` governs the audit and the fix sequence. Once you know what you are building, a second profile (A1 through A7) governs the visual language of the replacement. A4 plus one other is legal.

### 1.3 IMAGE mode: pick the board type

| Deliverable | Profile | Install name |
|---|---|---|
| Website comps, hero, landing, multi-section | **B1** | `imagegen-frontend-web` |
| Mobile app screens and flows, iOS, Android, cross-platform | **B2** | `imagegen-frontend-mobile` |
| Brand kit boards, logo directions, palettes, identity | **B3** | `brandkit` |

These output images only. They never write code. If the user wants code after the images, that is PIPELINE mode, not IMAGE mode.

### 1.4 Cross-mode conflict

If two modes are somehow both active and their rules collide, the Conflict Register (Part 4) rules. If the Register is silent, the DEFAULT profile A1 rules. If A1 is silent, ask one question. Do not invent a resolution.

---

## PART 2: UNIVERSAL INVARIANTS

Every module in this corpus agrees on the rules below, or is silent on them and contradicted by none. They fire in every mode. Where a single module carries a documented override, the override is printed inline and nowhere else.

### 2.1 Colour

* **Never pure black `#000000`.** Off-black, zinc-950, charcoal, or `#0A0A0A` to `#121212`. Pure black kills depth. (Agreed by A1, A2, A5, A6, A7, C1, and the redesign audit. A7 states it independently for its dark substrate.)
* **Never pure white `#ffffff` as page background in dark-mode-capable work.** Off-white. (A1.)
* **Maximum 1 accent colour. Saturation below 80%.** (A1, A2, C1. A7 complies independently with a single hazard red.)
* **No AI-purple / violet / neon-glow gradient aesthetic as a default reach.** Neutral bases (Zinc, Slate, Stone) with one high-contrast accent. **Override:** if the brand explicitly is purple, embrace it and execute with intent. (A1, A2, A4, C1.)
* **One palette per project.** No fluctuating between warm and cool greys inside one project. (A1, A2, A4, C1.)
* **Colour Consistency Lock.** Once the accent is chosen it is used on the whole page. No blue CTA in section 7 of a warm-grey site. Audit every component before shipping. (A1.)
* **Tint shadows to the background hue.** No pure-black drop shadows on light backgrounds. (A1, A2, A4.)

### 2.2 Typography

* **Body copy max-width 65 characters (`max-w-[65ch]`).** (A1, A2, A4, C1.)
* **No oversized H1 that just screams.** Control hierarchy with weight and colour, not raw scale. (A1, A2.)
* **Numbers go monospace or tabular-figures in data-dense contexts** (`font-variant-numeric: tabular-nums`). Mandatory when density exceeds 7. (A1, A2, A4, C1.)
* **No excessive gradient text on large headers.** (A1, A2, C1.)

### 2.3 Layout mechanics

* **Never `h-screen` or `height: 100vh` for full-height sections. Always `min-h-[100dvh]`.** iOS Safari address-bar collapse causes catastrophic layout jump. (A1, A2, A4, A5, C1. No module dissents.)
* **CSS Grid over flexbox percentage math.** `w-[calc(33%-1rem)]` is banned. Use `grid grid-cols-1 md:grid-cols-3 gap-6`. (A1, A2, C1.)
* **Contain page layouts.** `max-w-[1400px] mx-auto` or `max-w-7xl`. (A1, A2, A4, C1.)
* **Mobile collapse below 768px is explicit and per-section.** Every multi-column layout declares its `< 768px` fallback in the same component. `w-full`, `px-4`. No "Tailwind will handle it" assumptions. (A1, A2, A5, C1.)
* **Horizontal scroll on mobile is a critical failure**, not a cosmetic one. (C1, A3.)
* **No `z-index` spam.** No arbitrary `z-50`, no `z-[9999]`. Z-index is reserved for systemic layers: sticky nav, modal, overlay, grain. Document the scale in a constants file. (A1, A2, A4, C1.)
* **Standard breakpoints:** `sm 640`, `md 768`, `lg 1024`, `xl 1280`, `2xl 1536`. (A1.)

### 2.4 Motion and performance

* **Animate `transform` and `opacity` only.** Never `top`, `left`, `width`, `height`. These trigger layout and destroy frame budget. (A1, A2, A4, A5, A6, C1. Universal, no dissent.)
* **`window.addEventListener('scroll')` is a hard ban.** Not a preference. Use `IntersectionObserver`, Motion's `useScroll()`, GSAP `ScrollTrigger`, or CSS `animation-timeline: view()`. (A1, A2, A5, A6.)
* **Never drive continuous input values through React `useState`** (mouse position, scroll progress, pointer physics, magnetic hover). Use `useMotionValue` / `useTransform` outside the render cycle. `useState` re-renders on every frame and collapses on mobile. (A1, A2.)
* **`will-change: transform` sparingly**, only on elements actively animating. (A1, A2, A5, A6.)
* **Grain and noise filters apply exclusively to fixed, `pointer-events-none` pseudo-elements** (`fixed inset-0 pointer-events-none`). Never on scrolling containers. Continuous GPU repaint destroys mobile FPS. (A1, A2, A5, C1.)
* **`backdrop-filter` / `backdrop-blur` only on fixed or sticky elements** (navbars, overlays). Never on scrolling containers or large content areas. (A5.)
* **Isolate motion in client leaf components.** `'use client'` at the top, memoized, no parent re-renders. Server Components render static layouts only. (A1, A2, C1.)
* **`useEffect` animations carry strict cleanup functions.** GSAP work uses `gsap.context()` and `ctx.revert()`. (A1, A2.)
* **`prefers-reduced-motion` is honoured for any motion above intensity 3.** Infinite loops, parallax, scroll-hijack, and magnetic physics collapse to static. Non-negotiable. (A1.)
* **Never mix GSAP or Three.js with Motion in the same component tree.** They fight over the same frames. Isolate each in dedicated leaves. (A1, A2.)

### 2.5 Interactive states

* **Loading, empty, and error states are mandatory, not optional.** LLMs default to shipping the static successful state only. (A1, A2, A4, C1.)
* **Skeletal loaders that match the final layout's shape and radius.** Never generic circular spinners. (A1, A2, A4, A6, C1.)
* **Empty states are composed** and indicate how to populate. Never a bare "No data found". (A1, A2, A4, C1.)
* **Error states are inline and contextual.** Never `window.alert()`. (A1, A2, A4, C1.)
* **Tactile feedback on `:active`.** `scale(0.98)` or `-translate-y-[1px]` to simulate physical press. (A1, A2, A5, A6, C1.)
* **Visible focus ring for keyboard navigation.** This is an accessibility requirement, not a style choice. (A4.)
* **Touch targets minimum 44px.** (C1.)
* **Hover states on every interactive element.** No dead interactivity. (A4.)

### 2.6 Forms

* **Label above the input. Helper text optional but present in markup. Error text below the input.** Standard `gap-2`. (A1, A2, C1.)
* **No placeholder-as-label. Ever.** (A1.)
* **Form contrast passes WCAG AA** against the section background: inputs, placeholders, focus rings, helper text, error text. (A1.)
* **Client-side validation exists** for emails, required fields, formats. (A4.)

### 2.7 Content

* **No generic placeholder names.** "John Doe", "Jane Smith", "Sarah Chan", "Jack Su" are banned. Use creative, realistic, locale-appropriate names. (A1, A2, A4, A6, C1.)
* **No startup-slop brand names.** "Acme", "Nexus", "SmartFlow", "Cloudly" are banned. Invent contextual names that sound real. (A1, A2, A4, A6, C1.)
* **No fake-perfect numbers.** `99.99%`, `50%`, `1234567` are banned. Use organic, messy data: `47.2%`, `+1 (312) 847-1928`. (A1, A2, A4, C1.)
* **No Lorem Ipsum.** Write real draft copy. (A4, A6.)
* **No AI copywriting cliches.** "Elevate", "Seamless", "Unleash", "Next-Gen", "Game-changer", "Revolutionize", "Delve", "Tapestry", "In the world of". Concrete verbs only. (A1, A2, A4, A6, C1.)
* **No generic avatars.** No SVG "egg", no Lucide user icon. Believable photo placeholders or specific styling. (A1, A2, A4.)
* **Unique avatar per distinct person.** (A4.)
* **Randomize dates.** All blog posts dated identically is a tell. (A4.)

### 2.8 Assets

* **No broken Unsplash links.** Use `https://picsum.photos/seed/{descriptive-seed}/{w}/{h}`, generated images, or real brand assets. **Override (A1 only):** open-license sources including Unsplash direct URLs are acceptable when explicitly allowed by the brief. (A1, A2, A4, A6, C1.)
* **Missing favicon is a defect.** Always include a branded favicon. (A4.)
* **Missing alt text is a defect.** Describe content. Never `alt=""` or `alt="image"` on meaningful images. (A4.)
* **Standardize icon `strokeWidth` globally** across every icon in the project. One value, e.g. `1.5` or `2.0`. (A1, A2, A4.)
* **One icon family per project.** Do not mix Phosphor with Lucide in the same tree. (A1.)

### 2.9 Engineering hygiene

* **Dependency verification is mandatory.** Before importing any third-party library, check `package.json`. If missing, output the install command first. Never assume a library exists. (A1, A2, A4.)
* **Check the Tailwind version before touching config.** v3 and v4 syntax are not interchangeable. For v4, do not use the `tailwindcss` plugin in `postcss.config.js`; use `@tailwindcss/postcss` or the Vite plugin. (A1, A2, A4.)
* **Work with the existing stack.** Do not migrate frameworks or styling libraries mid-task. (A4.)
* **shadcn/ui is never shipped in its default state.** Customize radii, colours, shadows, typography to the project aesthetic. (A1, A2, C1.)
* **One design system per project.** No Fluent React beside Carbon. No shadcn imported into a Material 3 app. (A1.)
* **Semantic HTML.** `<nav>`, `<main>`, `<article>`, `<aside>`, `<section>`. No div soup. (A4.)
* **No commented-out dead code** in shipped output. (A4.)
* **Verify every import exists** in the dependency file. Import hallucination is a defect. (A4.)
* **Meta tags present:** `<title>`, `description`, `og:image`, social sharing. (A4.)

### 2.10 The em-dash ban

**The em-dash (`—`) and the en-dash (`–`) are banned in all generated user-facing output.** Headlines, eyebrows, pills, body copy, quotes, attribution, captions, button text, alt text, nav items. Zero. Not "sparingly". Not "in body copy is fine". Zero.

Permitted dash characters: the regular hyphen `-` (compound words, ranges, dividers) and the minus sign in maths (`-5°C`).

Replace with: a period, a comma, a colon, parentheses, a line break, or a restructured sentence. Date ranges use a hyphen (`2018-2026`). Number ranges use a hyphen (`€40-80k`).

A single `—` or `–` visible to the user is a Pre-Flight failure and the output must be rewritten. (A1, Section 9.G, stated as the single most-violated tell.)

**Note on this file:** the verbatim source profiles in Part 5 onward contain em-dashes in their own prose. They are preserved unaltered as source material. The ban governs **what you generate**, not the archived text of the skills themselves.

### 2.11 The anti-default reflex

Do not default to: AI-purple gradients, centered hero over dark mesh, three equal feature cards, generic glassmorphism on everything, infinite-loop micro-animations everywhere, Inter plus slate-900, div-based fake screenshots, Lorem Ipsum, "Elevate your workflow".

These are the LLM statistical defaults. Every one of them is what the model reaches for when it has not read the brief. Reach past them deliberately, based on the design read, not reflexively. (A1, A2, A3, A4, A5, A6, C1. This is the thesis of the entire corpus.)

---

## PART 3: THE DIALS

Four of the modules run on the same three dials. They are the shared control surface of this file.
...
**Baseline: `8 / 6 / 4`.** Every module that uses dials ships this same baseline. Use it unless the design read overrides.

Never ask the user to edit this file. Overrides happen conversationally.

Never invent aliases. The variable names are exactly `DESIGN_VARIANCE`, `MOTION_INTENSITY`, `VISUAL_DENSITY`. Not `LAYOUT_VARIANCE`, not `ANIM_LEVEL`.

**Naming note.** The Stitch profile (C1) and its generated `DESIGN.md` use a four-dial variant: `Creativity 8`, `Density 4`, `Variance 8`, `Motion Intent 6`. `Creativity` has no equivalent in the three-dial system. When crossing between CODE and SPEC modes, map `Variance → DESIGN_VARIANCE`, `Motion Intent → MOTION_INTENSITY`, `Density → VISUAL_DENSITY`, and treat `Creativity` as a Stitch-only dial governing inline-image typography and editorial expressiveness.

Full dial definitions, inference tables, and use-case presets live in Profile A1 Sections 1 and 7, and Profile A2 Sections 1 and 6.

---

## PART 4: CONFLICT REGISTER

Fourteen documented contradictions across the corpus. Each row names the ruling, the dissent, and the condition that flips it. Nothing here is deleted. The dissenting rule is preserved verbatim in its own profile and governs when its profile is active.

### C-01: The Inter font

* **Ruling:** Inter is discouraged as a default. Reach for `Geist`, `Outfit`, `Cabinet Grotesk`, `Satoshi`, or a brand-appropriate alternative first.
* **Dissent:** **A7 (brutalist)** lists `Inter (Extra Bold/Black)` as an optimal macro-typography face. **A3, A5, C1, A2** ban it outright by name.
* **Flips when:** A7 is the active profile and the weight is Extra Bold or Black for structural macro-typography. Also flips under A1's printed override: Inter is acceptable when the user explicitly asks for a neutral, standard, or Linear-style feel, or when the brief is public-sector or accessibility-first.
* **Reasoning:** A7 uses Inter Black as a Neo-Grotesque architectural block, which is a legitimate and different use from Inter Regular as a lazy body default. The ban targets the reflex, not the typeface.

### C-02: Corner radius

* **Ruling:** **Shape Consistency Lock.** Pick ONE radius scale for the page and hold it. All-sharp (0), all-soft (12-16px), or all-pill (full radius on interactive). Mixed systems are legal only when the rule is documented and followed everywhere.
* **Dissent:** **A7** demands absolute rejection of `border-radius`, 90 degrees on every corner. **A5** demands `rounded-[2rem]` containers and `rounded-full` pill buttons. **A6** caps radius at 8-12px for cards, 4-6px for buttons, and bans `rounded-full` for large containers and primary buttons. **C1** specifies `2.5rem` cards.
* **Flips when:** any of A5, A6, A7, C1 is the active profile. Its radius rule is absolute inside its own profile.
* **Reasoning:** these are the three profiles' core identity. There is no compromise radius. Pick a profile, inherit its geometry.

### C-03: Centered hero

* **Ruling:** Centered hero is avoided when `DESIGN_VARIANCE > 4`. Force split-screen, left-aligned content with right asset, asymmetric white-space, or scroll-pinned.
* **Dissent:** **A3 (gpt-taste)** names *Cinematic Center* as "Highly Preferred" and its first hero option.
* **Flips when:** A3 is the active profile. Also flips under A1's printed override: centered hero is correct for editorial, manifesto, and launch-announcement briefs where the message itself is the design.
* **Reasoning:** A3's centered hero is a full-bleed cinematic composition with a dark radial wash, not the centered-text-on-mesh default the ruling exists to kill. Different artefact, same word.

### C-04: Animation library

* **Ruling:** **Motion** (`import { motion } from "motion/react"`) is the default for UI, bento, and state-change motion. **GSAP + ScrollTrigger** for full-page scrolltelling and scroll hijacks, isolated in dedicated leaves. **Three.js** for canvas and 3D, same isolation.
* **Dissent:** **A2 (v1)** defaults to Framer Motion and treats GSAP as the exception. **A3** mandates real GSAP (`@gsap/react`, `ScrollTrigger`) and barely mentions Framer.
* **Flips when:** A3 is active (GSAP is mandatory there). A2 is active (Framer Motion is the default there; note `framer-motion` still works as a legacy alias for `motion/react`).
* **Never flips:** the ban on mixing GSAP or Three.js with Motion in the same component tree is an invariant (2.4) and holds in every profile.

### C-05: Serif typography

* **Ruling:** Serif is **very discouraged** as a default. "It feels creative or premium" is not a reason. Default to sans display. Serif is acceptable only when the brand brief names a serif, or the family is genuinely editorial, luxury, publication, manuscript, heritage, or vintage AND you can articulate why this specific serif fits this specific brand. `Fraunces` and `Instrument_Serif` are **specifically banned as defaults**, being the two LLM-favourite display serifs.
* **Dissent:** **C1 (stitch)** bans generic serifs (Times, Georgia, Garamond, Palatino) but explicitly **recommends `Fraunces` and `Instrument Serif`** as the distinctive modern alternatives. This is a direct collision with A1's specific ban on exactly those two faces. **A6** mandates an editorial serif for hero headings. **A7** uses high-contrast serif as textural disruption, heavily halftoned.
* **Flips when:** C1, A6, or A7 is active, each inside its own profile.
* **Reasoning:** C1 predates A1's production testing. A1 banned Fraunces and Instrument Serif precisely because skills like C1 recommended them until every AI site used them. If you are in SPEC mode writing a `DESIGN.md`, prefer A1's rotation pool over C1's two names, and say so in the spec.
* **Universal regardless of profile:** serif is banned in dashboards and software UI. Every module agrees.

### C-06: Eyebrow labels

* **Ruling:** **Maximum 1 eyebrow per 3 sections.** Hero counts as 1. A 9-section page gets at most 3. If section A has one, the next two cannot. The mechanical check: count `uppercase tracking` micro-labels; if count exceeds `ceil(sectionCount / 3)`, the output fails. Named by A1 as "the #1 violated rule in production tests".
* **Dissent:** **A5 (soft)** mandates the opposite: "Precede major H1/H2s with a microscopic, pill-shaped badge (`rounded-full px-3 py-1 text-[10px] uppercase tracking-[0.2em]`)".
* **Flips when:** A5 is active. Even then, prefer A1's cap. A5's eyebrow rule is the single most dangerous rule in this corpus because it mandates by name the exact pattern A1 identifies as the most common AI tell.
* **Recommended reconciliation:** when running A5, apply its eyebrow *styling* to the eyebrows you do use, and apply A1's *count cap* to how many you use.

### C-07: Perpetual micro-animation

* **Ruling:** **Not every card needs an infinite loop.** If a section is informational, leave it still. Motion must be motivated: hierarchy, storytelling, feedback, or state transition. "It looked cool" is not an answer. If you cannot justify the animation in one sentence, drop it.
* **Dissent:** **A2** and **C1** both mandate that *every* active component carries an infinite loop state (Pulse, Typewriter, Float, Shimmer, Carousel). A2 Section 9.B: "Every card must have an 'Active State' that loops infinitely to ensure the dashboard feels alive."
* **Flips when:** A2 or C1 is active AND the surface is a dashboard or bento feature grid, which is the context both rules were written for.
* **Reasoning:** A2 and C1 scope their rule to dashboards and bento grids. A1 scopes its rule to landing pages and portfolios. Read as scoped, they barely conflict. Read as universal, they contradict flatly. Scope them.

### C-08: Element overlap

* **Ruling (C1):** No overlapping elements, ever. Text never overlaps images. No z-index stacking of content layers. No absolute-positioned headlines over images. Clean spatial separation always.
* **Dissent:** **A3** hero option 2 is "Artistic Asymmetry: an artistic floating image overlapping the text from the bottom right". **A5** Layout Archetype 2 is "The Z-Axis Cascade: elements stacked like physical cards, slightly overlapping, some with subtle rotation". **A4** lists "No overlap or depth" as a defect to fix, prescribing negative margins to create layering.
* **Flips when:** A3, A4, or A5 is active. C1's no-overlap rule is a Stitch constraint (Stitch generates static screens and handles overlap badly), not a universal design truth.
* **Mobile invariant regardless:** A5 requires all overlaps and rotations be removed below 768px because overlapping elements cause touch-target conflicts. This holds in every profile.

### C-09: Glassmorphism

* **Ruling:** Contextual. Appropriate for premium consumer, Apple-adjacent, luxury, media-overlay. Inappropriate for dashboards, public-sector, boring B2B. When used, go past `backdrop-blur`: 1px inner border and inner shadow for edge refraction, plus a solid-fill fallback under `prefers-reduced-transparency`.
* **Dissent:** **A6** bans 3D glassmorphism outright, permitting only subtle navbar blurs. **A5** makes "Ethereal Glass" one of three primary vibe archetypes.
* **Flips when:** A6 is active (banned) or A5 selects Ethereal Glass (mandatory).
* **Universal regardless:** there is no official Apple `liquid-glass.css` for the web. Any web implementation is an approximation using `backdrop-filter` plus layered borders plus highlights, and must be labelled as an approximation in code comments. See Appendix C.

### C-10: Gradients

* **Ruling:** No AI-purple gradients. Break uniformity with radial, mesh, or noise rather than standard 45-degree linear fades. Brand-appropriate gradients in bento cells are encouraged for background diversity.
* **Dissent:** **A6** bans gradients outright. **A7** bans gradients outright ("Gradients, soft drop shadows, and modern translucency are strictly prohibited").
* **Flips when:** A6 or A7 is active. Both bans are absolute inside their profiles.

### C-11: Icon libraries

* **Ruling:** Allowed in priority order: `@phosphor-icons/react`, `hugeicons-react`, `@radix-ui/react-icons`, `@tabler/icons-react`. `lucide-react` is discouraged, acceptable only on explicit request or existing dependency. **Never hand-roll SVG icon paths.**
* **Dissent:** **A4** recommends "Phosphor, Heroicons, or a custom set". **A6** bans "Lucide, Feather, or standard Heroicons" and mandates Phosphor Bold/Fill or Radix. **A5** bans "standard thick-stroked Lucide, FontAwesome, Material" and mandates ultra-light precise lines (Phosphor Light, Remix Line).
* **Resolution:** Phosphor satisfies every profile and is the safe default across the whole corpus. A4's Heroicons suggestion is overruled by A6's ban. A5's weight preference (Light) and A6's weight preference (Bold/Fill) are profile-scoped, not contradictory, since they never run together.
* **Never flips:** the ban on hand-rolled SVG icon paths, and the global `strokeWidth` standardization, are invariants (2.8).

### C-12: Uppercase and all-caps

* **Ruling:** All-caps subheaders everywhere is a defect. Try lowercase italics, sentence case, or small-caps. Title Case On Every Header is a defect; use sentence case.
* **Dissent:** **A7** mandates exclusively uppercase for both macro-typography (structural impact) and micro-typography (all metadata, navigation, unit IDs, coordinates).
* **Flips when:** A7 is active. Uppercase is A7's structural language, not a lazy default.

### C-13: Card usage

* **Ruling:** Use cards ONLY when elevation communicates real hierarchy. Otherwise group with `border-t`, `divide-y`, or negative space. For `VISUAL_DENSITY > 7`, generic card containers are banned outright.
* **Dissent:** **A6** mandates bento cards with `border: 1px solid #EAEAEA` and 24-40px internal padding as a core component. **A4** lists "generic card look (border + shadow + white background)" as a defect. **A5** mandates the Double-Bezel nested card architecture for all major containers.
* **Flips when:** A5 or A6 is active. Their card architectures are their identity.
* **Reasoning:** the ruling is about card *overuse* as a default reach. A5 and A6 use cards deliberately as a designed system, which is the ruling's own stated exception.

### C-14: Pre-flight ritual

* **Ruling:** Output the **Design Read** one-liner before any code (A1 Section 0.B), then run the Final Pre-Flight Check (Part 9).
* **Dissent:** **A3** mandates a `<design_plan>` block containing a simulated Python RNG execution seeded on the prompt's character count, selecting hero architecture, typography stack, component architectures, and GSAP paradigms.
* **Flips when:** A3 is active. Run A3's `<design_plan>` instead of, not in addition to, the Design Read.
* **Reasoning:** both rituals solve the same problem (the model picking the first option every time) with different mechanisms. Running both is redundant ceremony. Pick the one belonging to your profile.

### Conflict-free by scope

The following look like conflicts and are not. They are different modes that never run together: IMAGE profiles (B1, B2, B3) output images and never code. PIPELINE (D1) sequences images then code. SPEC (C1) outputs a `DESIGN.md` and never code. A8 (`full-output-enforcement`) is cross-cutting and contradicts nothing.

---

## PART 5: CODE MODE PROFILES

Seven aesthetic profiles plus one cross-cutting enforcement profile. Pick exactly one of A1 through A7 per project. A8 stacks on any of them. Every profile below is the complete source skill, verbatim.

---

### PROFILE A1: design-taste-frontend (v2). THE DEFAULT

> **Install name:** `design-taste-frontend`
> **Source:** `skills/taste-skill/SKILL.md`, 1201 lines, reproduced verbatim.
> **Original description:** Anti-slop frontend skill for landing pages, portfolios, and redesigns. The agent reads the brief, infers the right design direction, and ships interfaces that do not look templated. Real design systems when applicable, audit-first on redesigns, strict pre-flight check.
...
### PROFILE A2: design-taste-frontend-v1

> **Install name:** `design-taste-frontend-v1`
> **Source:** `skills/taste-skill-v1/SKILL.md`, 221 lines, reproduced verbatim.
> **Original description:** The original v1 taste-skill, preserved for projects depending on its exact behavior. The current default is `design-taste-frontend` (v2 experimental), which is a substantial rewrite. Use this v1 install name only if you need exact backward compatibility.
...
### PROFILE A3: gpt-taste

> **Install name:** `gpt-taste`
> **Source:** `skills/gpt-tasteskill/SKILL.md`, 69 lines, reproduced verbatim.
> **Original description:** Elite UX/UI & Advanced GSAP Motion Engineer. Enforces Python-driven true randomization for layout variance, strict AIDA page structure, wide editorial typography (bans 6-line wraps), gapless bento grids, strict GSAP ScrollTriggers (pinning, stacking, scrubbing), inline micro-images, and massive section spacing.
...
### PROFILE A4: redesign-existing-projects

> **Install name:** `redesign-existing-projects`
> **Source:** `skills/redesign-skill/SKILL.md`, 173 lines, reproduced verbatim.
> **Original description:** Upgrades existing websites and apps to premium quality. Audits current design, identifies generic AI patterns, and applies high-end design standards without breaking functionality. Works with any CSS framework or vanilla CSS.
...
### PROFILE A5: high-end-visual-design

> **Install name:** `high-end-visual-design`
> **Source:** `skills/soft-skill/SKILL.md`, 93 lines, reproduced verbatim.
> **Original description:** Teaches the AI to design like a high-end agency. Defines the exact fonts, spacing, shadows, card structures, and animations that make a website feel expensive. Blocks all the common defaults that make AI designs look cheap or generic.
...
### PROFILE A6: minimalist-ui

> **Install name:** `minimalist-ui`
> **Source:** `skills/minimalist-skill/SKILL.md`, 80 lines, reproduced verbatim.
> **Original description:** Clean editorial-style interfaces. Warm monochrome palette, typographic contrast, flat bento grids, muted pastels. No gradients, no heavy shadows.
...
### PROFILE A7: industrial-brutalist-ui

> **Install name:** `industrial-brutalist-ui`
> **Source:** `skills/brutalist-skill/SKILL.md`, 87 lines, reproduced verbatim.
> **Original description:** Raw mechanical interfaces fusing Swiss typographic print with military terminal aesthetics. Rigid grids, extreme type scale contrast, utilitarian color, analog degradation effects. For data-heavy dashboards, portfolios, or editorial sites that need to feel like declassified blueprints.
...
### PROFILE A8: full-output-enforcement (cross-cutting)

> **Install name:** `full-output-enforcement`
> **Source:** `skills/output-skill/SKILL.md`, 44 lines, reproduced verbatim.
> **Original description:** Overrides default LLM truncation behavior. Enforces complete code generation, bans placeholder patterns, and handles token-limit splits cleanly. Apply to any task requiring exhaustive, unabridged output.
...
## PART 6: IMAGE MODE PROFILES

These produce design reference images only. They never write code. If code is wanted after the images, that is PIPELINE mode (Part 7), not IMAGE mode.

---

### PROFILE B1: imagegen-frontend-web

> **Install name:** `imagegen-frontend-web`
> **Source:** `skills/imagegen-frontend-web/SKILL.md`, 982 lines, reproduced verbatim.
> **Original description:** Elite frontend image-direction skill for generating premium, conversion-aware website design references. CRITICAL OUTPUT RULE — generate ONE separate horizontal image FOR EVERY section. A landing page with 8 sections produces 8 images. Never compress multiple sections into one image. Enforces composition variety (not always left-text / right-image), background-image freedom, varied CTAs, varied hero scales (giant / mid / mini minimalist), narrative concept spine, second-read moments, and a single consistent palette across all images. Optimized for landing pages, marketing sites, and product comps that developers or coding models can accurately recreate.
...
### PROFILE B2: imagegen-frontend-mobile

> **Install name:** `imagegen-frontend-mobile`
> **Source:** `skills/imagegen-frontend-mobile/SKILL.md`, 1460 lines, reproduced verbatim.
> **Original description:** Elite mobile app image-generation skill for creating premium, app-native screen concepts and flows. Designed for iOS, Android, and cross-platform mobile products. Prioritizes clean hierarchy, comfortably readable text, strong multi-screen consistency, controlled color palettes, non-generic creative direction, textured surfaces, image-led composition, tasteful custom iconography, and clean phone mockup framing. By default, screens should be shown inside a subtle premium iPhone or similar phone mockup with a visible frame, while the main focus stays on the app content itself. This skill generates images only. It does not write code.
...
> **Source:** `skills/brandkit/SKILL.md`, 793 lines, reproduced verbatim.
> **Original description:** Premium brand-kit image generation skill for creating high-end brand-guidelines boards, logo systems, identity decks, and visual-world presentations. Trained for minimalist, cinematic, editorial, dark-tech, luxury, cultural, security, gaming, developer-tool, and consumer-app brand systems. Optimized for intentional logo concepting, refined composition, sparse typography, strong symbolic meaning, premium mockups, art-directed imagery, and flexible grid layouts.
...
## PART 7: PIPELINE MODE PROFILE

Image-first: generate references, analyse them deeply, then implement code to match. State the pipeline in the prompt, e.g. `follow the skill: generate images, then analyze, then code`.

---

### PROFILE D1: image-to-code

> **Install name:** `image-to-code`
> **Source:** `skills/image-to-code-skill/SKILL.md`, 1223 lines, reproduced verbatim.
> **Original description:** Elite website image-to-code skill for Codex. For visually important web tasks, it must first generate the design image(s) itself, deeply analyze them, then implement the website to match them as closely as possible. In Codex, it must prefer large, readable, section-specific images instead of tiny compressed boards, generate fresh standalone images for sections or detail views instead of cropping old ones, avoid lazy under-generation, avoid cards-inside-cards-inside-cards UI, and keep the hero clean, spacious, readable, and visible on a small laptop.
...
## PART 8: SPEC MODE PROFILE

Generates a `DESIGN.md` file as the single source of truth for Google Stitch or another downstream agent. Outputs a spec, never code.

---

### PROFILE C1: stitch-design-taste

> **Install name:** `stitch-design-taste`
> **Source:** `skills/stitch-skill/SKILL.md`, 179 lines, reproduced verbatim.
> **Original description:** Semantic Design System Skill for Google Stitch. Generates agent-friendly DESIGN.md files that enforce premium, anti-generic UI standards — strict typography, calibrated color, asymmetric layouts, perpetual micro-motion, and hardware-accelerated performance.
...
### PROFILE C1 REFERENCE OUTPUT: the generated `DESIGN.md`

> **Source:** `skills/stitch-skill/DESIGN.md`, 121 lines, reproduced verbatim.
> This is a worked example of C1's output at the baseline dials, not an input rule. It is the artefact SPEC mode produces.
...
## PART 9: UNIFIED PRE-FLIGHT CHECK

**This is not optional. Run every box. If a single box cannot be honestly ticked, the output is not done.**

Run this check, then run your active profile's own check on top of it. The profile checks live verbatim inside the profiles and are not duplicated here:

| Profile | Its own check | Location |
|---|---|---|
| A1 | Final Pre-Flight Check, ~60 boxes | Profile A1, Section 14 |
| A2 | Final Pre-Flight Check, 7 boxes | Profile A2, Section 10 |
| A3 | Mandatory Pre-Flight `<design_plan>`, 5 items, runs **before** code | Profile A3, Section 8 |
| A4 | Fix Priority, 7 ordered steps | Profile A4, Fix Priority |
| A5 | Pre-Output Checklist, 11 boxes | Profile A5, Section 8 |
| A6 | Execution Protocol, 7 steps | Profile A6, Section 8 |
| A7 | Web Engineering Directives, 3 items | Profile A7, Section 8 |
| A8 | Quick Check, 4 items | Profile A8, Quick Check |
| B1 | Pre-output check | Profile B1, Section 4 |
| B2 | Quality Check, Section 35 | Profile B2, Section 35 |
| D1 | Clarity Check, Section 35 | Profile D1, Section 35 |

### 9.1 Routing

- [ ] **Deliverable identified** (CODE / IMAGE / PIPELINE / SPEC / AUDIT) before anything was produced?
- [ ] **Exactly one** aesthetic profile from A1 through A7 is active? (A8 and A4 may stack. A5 and A7 may not.)
- [ ] **Design Read declared** (A1 Section 0.B one-liner), OR A3's `<design_plan>` block emitted if A3 is the active profile? Per C-14, one ritual, not both.
- [ ] **Dial values explicit and reasoned** from the brief, not silently inherited from the `8 / 6 / 4` baseline?
- [ ] **Conflict Register consulted** for every row touching the active profile?

### 9.2 The invariants (Part 2, every mode, no exceptions)

- [ ] **ZERO em-dashes (`—`) and zero en-dashes (`–`)** anywhere user-facing. Headlines, eyebrows, pills, body, quotes, attribution, captions, buttons, alt text, nav. Zero.
- [ ] **No pure `#000000`**, and no pure `#ffffff` background in dark-mode-capable work?
- [ ] **One accent colour**, saturation below 80%, used identically across every section?
- [ ] **One palette.** No warm greys beside cool greys?
- [ ] **`min-h-[100dvh]`**, never `h-screen` or `100vh`?
- [ ] **CSS Grid, not flex percentage math.** No `w-[calc(33%-1rem)]`?
- [ ] **Mobile collapse declared explicitly per section** below 768px, and zero horizontal scroll at 375px?
- [ ] **Animations touch only `transform` and `opacity`?**
- [ ] **No `window.addEventListener('scroll')`** anywhere?
- [ ] **No `useState` driving continuous input values** (mouse, scroll, pointer physics)?
- [ ] **`prefers-reduced-motion` honoured** for all motion above intensity 3?
- [ ] **Motion isolated** in `'use client'` leaf components, memoized, with `useEffect` cleanup?
- [ ] **GSAP / Three.js never mixed with Motion** in the same component tree?
- [ ] **Grain, noise, and `backdrop-blur` only on fixed or sticky, `pointer-events-none` layers?**
- [ ] **Loading, empty, AND error states** all present?
- [ ] **Skeletal loaders**, never circular spinners?
- [ ] **Tactile `:active` feedback** and a visible focus ring on every interactive element?
- [ ] **Touch targets 44px minimum?**
- [ ] **Forms:** label above, error below, no placeholder-as-label, WCAG AA contrast on every part?
- [ ] **Button contrast** passes WCAG AA, no white-on-white, no invisible CTA text?
- [ ] **Content clean:** no John Doe, no Acme, no `99.99%`, no Lorem Ipsum, no "Elevate / Seamless / Unleash"?
- [ ] **Assets real:** no broken Unsplash, no hand-rolled SVG icon paths, no div-based fake screenshots, favicon present, alt text present?
- [ ] **One icon family**, one global `strokeWidth`?
- [ ] **Dependencies verified** against `package.json`, Tailwind version checked before touching config?
- [ ] **shadcn/ui customized**, never shipped in default state?
...
- [ ] **Page Theme Lock:** ONE theme (light, dark, or auto) for the whole page. No section inverts mid-scroll.
- [ ] **Colour Consistency Lock:** the accent chosen in section 1 is the accent in section 9.
- [ ] **Shape Consistency Lock:** one radius system, applied everywhere, per C-02 and the active profile.
- [ ] **Copy Register Lock:** one voice. Not technical mono beside editorial prose beside marketing punch.

### 9.4 Completeness (A8, apply always)

- [ ] **No banned truncation patterns:** no `// ...`, no `// rest of code`, no `// TODO`, no bare `...` standing in for omitted code, no "for brevity", no "the rest follows the same pattern"?
- [ ] **Deliverable count locked at scope and verified at output.** Asked for 5 components, shipped 5?
...
This file is a lossless consolidation. Every source is reproduced verbatim, byte-for-byte, with only two mechanical transforms: YAML frontmatter lifted into a provenance block, and markdown heading levels demoted by three so they nest under their mode headers. No source text was reworded, reordered, summarised, softened, or dropped.

| Profile | Install name | Source path | Lines |
|---|---|---|---|
| A1 | `design-taste-frontend` | `skills/taste-skill/SKILL.md` | 1,206 |
| A2 | `design-taste-frontend-v1` | `skills/taste-skill-v1/SKILL.md` | 226 |
| A3 | `gpt-taste` | `skills/gpt-tasteskill/SKILL.md` | 74 |
| A4 | `redesign-existing-projects` | `skills/redesign-skill/SKILL.md` | 178 |
| A5 | `high-end-visual-design` | `skills/soft-skill/SKILL.md` | 98 |
| A6 | `minimalist-ui` | `skills/minimalist-skill/SKILL.md` | 85 |
| A7 | `industrial-brutalist-ui` | `skills/brutalist-skill/SKILL.md` | 92 |
| A8 | `full-output-enforcement` | `skills/output-skill/SKILL.md` | 49 |
| B1 | `imagegen-frontend-web` | `skills/imagegen-frontend-web/SKILL.md` | 987 |
| B2 | `imagegen-frontend-mobile` | `skills/imagegen-frontend-mobile/SKILL.md` | 1,465 |
| B3 | `brandkit` | `skills/brandkit/SKILL.md` | 798 |
| C1 | `stitch-design-taste` | `skills/stitch-skill/SKILL.md` | 184 |
| C1 ref | (output artefact) | `skills/stitch-skill/DESIGN.md` | 129 |
| D1 | `image-to-code` | `skills/image-to-code-skill/SKILL.md` | 1,228 |

**Total source: 6,670 SKILL.md lines plus the 129-line DESIGN.md reference artefact.**

Upstream: `github.com/Leonxlnx/taste-skill`, MIT License, Copyright (c) 2026 Leonxlnx. The authored layer (Parts 0 through 4, 9, 10) is new work written for this consolidation.

**Known divergences inside the corpus, preserved not corrected:**

1. The verbatim profiles contain em-dashes in their own prose. Part 2.10 bans em-dashes in **generated output**, not in the archived skill text. Both statements stand.
2. C1 recommends `Fraunces` and `Instrument Serif`. A1 bans those two specifically. See C-05. Both are preserved as written.
3. A5 mandates an eyebrow above every major heading. A1 caps eyebrows at one per three sections and calls the pattern its most-violated tell. See C-06. Both are preserved as written.
4. A2 and C1 mandate an infinite loop on every card. A1 says not every card needs one. See C-07. Both are preserved as written.
5. A7 lists Inter as an optimal face. Five other profiles ban it. See C-01. Both are preserved as written.

If a future edit resolves any of these upstream, update the Conflict Register row, not the verbatim profile.